...
export default class Solicitacoes extends Component {
    ...
    buscarNome(page = 1) {        
        AlunoDataService.buscarNome(this.state.buscaNome, page)
        .then(response => {
            const { docs, ...info } = response.data 
            this.setState({
        ...
        if (alunos.length > 0 && (buscaCPF || buscaDtNascimento || buscaNome || buscaEscola || buscaStatus || buscaSerie || buscaTurno || buscaTurma)) { 
            mostrar = alunos.map((aluno, index) => {
                if (aluno.situacao === false) {
                    ...
                    <td style={{width: 5+'%', color: 'red'}}>{aluno.pontos}</td>
                    ...
                    return (
                    <tr key={index} className="bordalinha">
                        <td style={{width: 5+'%'}}><b>{aluno.pontos}</b></td> 
